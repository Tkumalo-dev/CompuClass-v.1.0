...
export default function ProfileScreen({ onLogout }) {
  ...
  const handleEditProfile = async () => {
    ...
    try {
      ...
      setShowEditModal(false); setAvatarFile(null); loadUser();
  ...
  const handleLogout = () => {
    ...
    Alert.alert('Logout', 'Are you sure you want to logout?', [
      ...
      { text: 'Logout', style: 'destructive', onPress: async () => {
        try { await authService.signOut(); onLogout(); }
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={{ paddingBottom: 100 + insets.bottom }} showsVerticalScrollIndicator={false}>
      ...
      <Modal visible={showEditModal} animationType="slide" transparent>
        <View style={styles.modalOverlay}>
          <View style={styles.modalCard}>
            ...
            <TouchableOpacity style={styles.avatarPickerBtn} onPress={handlePickAvatar}>
              <Ionicons name="camera-outline" size={20} color={BLUE} />
              <Text style={styles.avatarPickerText}>{avatarFile ? 'Avatar selected ✓' : 'Choose Avatar'}</Text>
            </TouchableOpacity>
            <TextInput style={styles.input} placeholder="Full Name" placeholderTextColor={MUTED} value={fullName} onChangeText={setFullName} />
            <View style={styles.modalBtns}>
              <TouchableOpacity style={styles.cancelBtn} onPress={() => setShowEditModal(false)} disabled={loading}>
      ...
      <Modal visible={showPasswordModal} animationType="slide" transparent>
        <View style={styles.modalOverlay}>
          <View style={styles.modalCard}>
            ...
            <View style={styles.modalBtns}>
              <TouchableOpacity style={styles.cancelBtn} onPress={() => setShowPasswordModal(false)} disabled={loading}>
                <Text style={styles.cancelBtnText}>Cancel</Text>
              </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  avatarWrap: { width: 96, height: 96, borderRadius: 48, borderWidth: 4, borderColor: WHITE, overflow: 'hidden', marginBottom: 12, backgroundColor: WHITE, alignItems: 'center', justifyContent: 'center' },
  ...
  actionCard: { flexDirection: 'row', alignItems: 'center', backgroundColor: WHITE, borderRadius: 16, padding: 16, gap: 14, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.06, shadowRadius: 6, elevation: 2 },
  ...
  avatarPickerBtn: { flexDirection: 'row', alignItems: 'center', borderWidth: 2, borderColor: BLUE, borderRadius: 12, padding: 12, marginBottom: 14, gap: 10, backgroundColor: BLUE + '10' },
  avatarPickerText: { color: BLUE, fontWeight: '700', fontSize: 14 },
  input: { backgroundColor: BG, borderWidth: 2, borderColor: BORDER, borderRadius: 12, padding: 14, marginBottom: 12, fontSize: 15, color: TEXT },
